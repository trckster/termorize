...
A Chromium Manifest V3 extension that translates selected text on any page and saves useful word pairs to the signed-in user's Termorize vocabulary. It also keeps its direct Google Translate workflow.
...
5. Pin the Termorize icon so the translation popup is always one click away.
