...
func TestTelegramDescriptionIgnoreAndUndoCallbacks(t *testing.T) {
	testkit.Truncate(t)
	tg := testkit.MockTelegramAPI(t)

	const telegramID int64 = 555094
	const exerciseMessageID int64 = 496
	user := testkit.CreateUser(t, testkit.WithTelegramID(telegramID), testkit.WithSettings(models.UserSettings{
		SystemLanguage:       enums.LanguageEn,
		MainLearningLanguage: enums.LanguageEn,
	}))
	...
	callback := func(id, action, text string) map[string]any {
		return map[string]any{
			"update_id": 1,
			"callback_query": map[string]any{
				"id":   id,
	...
	const question = "Guess the word described below in English:\n\nUsed for writing.\n\n(answer with reply)"
	rec := telegramUpdate(t, callback("description-ignore", "di", question))
	testkit.RequireStatus(t, rec, http.StatusOK)

	var storedUser models.User
	require.NoError(t, db.DB.Where("id = ?", user.ID).Take(&storedUser).Error)
	assert.Equal(t, []enums.Language{enums.LanguageEn}, storedUser.Settings.IgnoredDescriptionLanguages)
	require.Len(t, tg.RequestsFor("editMessageText"), 1)
	var cancelled telegramKeyboardRequest
	require.NoError(t, json.Unmarshal(tg.RequestsFor("editMessageText")[0].Body, &cancelled))
	assert.Contains(t, string(tg.RequestsFor("editMessageText")[0].Body), "cancelled")
	_, hasUndo := findCallbackButton(cancelled.ReplyMarkup.InlineKeyboard, "exercise:du:")
	assert.True(t, hasUndo)

	cancelledText := question + "\n\nThis description exercise was cancelled and is no longer scored."
	rec = telegramUpdate(t, callback("description-undo", "du", cancelledText))
	testkit.RequireStatus(t, rec, http.StatusOK)
	require.NoError(t, db.DB.Where("id = ?", user.ID).Take(&storedUser).Error)
...
func TestTelegramWebhookCharacterExerciseUsesSquareBoardAndCompletes(t *testing.T) {
	...
	assert.Contains(t, finalEdit["text"], telegram.GetBotTexts(enums.LanguageRu).ExerciseSuccess)
	assert.Contains(t, finalEdit["text"], "letter")
	assert.NotContains(t, finalEdit["text"], "＿")
	link := exerciseLink(t, exercise.ID, vocabulary.ID)
...
func TestTelegramWebhookExerciseIDKEditsOriginalMessage(t *testing.T) {
	for _, exerciseType := range []enums.ExerciseType{
		...
		t.Run(string(exerciseType), func(t *testing.T) {
			...
			require.NotNil(t, link.ProgressDelta)
			assert.Equal(t, services.ExerciseWrongProgressDeltaForType(exerciseType), *link.ProgressDelta)
			assert.Empty(t, tg.RequestsFor("editMessageReplyMarkup"))
			assert.Empty(t, tg.RequestsFor("sendMessage"))

			action, field := "editMessageText", "text"
			if exerciseType == enums.ExerciseTypeAudioDirect || exerciseType == enums.ExerciseTypeAudioReversed {
				action, field = "editMessageCaption", "caption"
			}
			require.Len(t, tg.RequestsFor(action), 1)
			var edit map[string]any
			require.NoError(t, json.Unmarshal(tg.RequestsFor(action)[0].Body, &edit))
			assert.Equal(t, float64(messageID), edit["message_id"])
			assert.Equal(t, float64(telegramID), edit["chat_id"])
			assert.Equal(t, "Markdown", edit["parse_mode"])
			assert.Empty(t, edit["reply_markup"].(map[string]any)["inline_keyboard"])
			assert.Contains(t, edit[field], "carta")
			assert.Contains(t, edit[field], "letter")
			assert.Contains(t, edit[field], enums.LanguageIt.Flag())
			assert.Contains(t, edit[field], enums.LanguageEn.Flag())
			assert.Contains(t, edit[field], telegram.GetBotTexts(enums.LanguageRu).ExerciseIDK)
		})
	}
}
...
func TestTelegramWebhookCompletedMatchTapRepairsOriginalMessage(t *testing.T) {
	...
	reply := telegramPrivateMessage(telegramID, "answer")
	reply["message"].(map[string]any)["reply_to_message"] = map[string]any{"message_id": messageID}
	testkit.RequireStatus(t, telegramUpdate(t, reply), http.StatusOK)
	require.Len(t, tg.RequestsFor("sendMessage"), 1)
	var sent struct {
		Text string `json:"text"`
	}
	require.NoError(t, json.Unmarshal(tg.RequestsFor("sendMessage")[0].Body, &sent))
	assert.Equal(t, telegram.GetBotTexts(enums.LanguageRu).ExerciseCompleted, sent.Text)
...
func TestTelegramWebhookExerciseRepliesDeliverResults(t *testing.T) {
	for _, exerciseType := range []enums.ExerciseType{
		enums.ExerciseTypeCharactersDirect, enums.ExerciseTypeCharactersReversed,
		enums.ExerciseTypeDescriptionDirect, enums.ExerciseTypeDescriptionReversed,
		enums.ExerciseTypeBasicDirect, enums.ExerciseTypeBasicReversed,
		...
		for _, verdict := range []string{"correct", "almost", "wrong"} {
			t.Run(string(exerciseType)+"/"+verdict, func(t *testing.T) {
				...
				exercise := exerciseSeedExercise(t, user.ID, exerciseType, enums.ExerciseStatusPending, vocabulary.ID)
				answer := "letter"
				order := []int{0, 1, 2, 3, 4, 5, -1, -1, -1}
				if exerciseType == enums.ExerciseTypeCharactersReversed || exerciseType == enums.ExerciseTypeBasicReversed || exerciseType == enums.ExerciseTypeAudioReversed || exerciseType == enums.ExerciseTypeDescriptionDirect {
					answer = "carta"
					order = []int{0, 1, 2, 3, 4, -1, -1, -1, -1}
				}
				isCharacters := exerciseType == enums.ExerciseTypeCharactersDirect || exerciseType == enums.ExerciseTypeCharactersReversed
				if isCharacters {
					require.NoError(t, services.StartCharacterExercise(exercise.ID, messageID, order))
					_, _, err := services.ApplyCharacterTap(exercise.ID, user.ID, 0)
					require.NoError(t, err)
				} else {
					require.NoError(t, db.DB.Model(&models.Exercise{}).Where("id = ?", exercise.ID).Updates(map[string]any{"telegram_message_id": messageID, "status": enums.ExerciseStatusInProgress}).Error)
				}
				status := enums.ExerciseStatusCompleted
				delta := services.ExerciseCharacterCorrectProgressDelta
				switch verdict {
				case "almost":
					answer = answer[:len(answer)-1]
					delta = services.ExerciseCharacterAlmostProgressDelta
				case "wrong":
					answer = "unrelated"
					status = enums.ExerciseStatusFailed
					delta = services.ExerciseCharacterWrongProgressDelta
				default:
					answer = "  " + answer + "  "
				}
				if !isCharacters {
				...
				update["message"].(map[string]any)["message_id"] = messageID + 1
				...
				assert.Equal(t, status, exerciseReload(t, exercise.ID).Status)
				link := exerciseLink(t, exercise.ID, vocabulary.ID)
				require.NotNil(t, link.Result)
				assert.Equal(t, verdict, *link.Result)
				require.NotNil(t, link.ProgressDelta)
				assert.Equal(t, delta, *link.ProgressDelta)
				require.Len(t, tg.RequestsFor("editMessageReplyMarkup"), 1)
				var cleanup struct {
					ChatID      int64 `json:"chat_id"`
					MessageID   int64 `json:"message_id"`
				...
				require.NoError(t, json.Unmarshal(tg.RequestsFor("editMessageReplyMarkup")[0].Body, &cleanup))
				assert.Equal(t, telegramID, cleanup.ChatID)
				assert.Equal(t, messageID, cleanup.MessageID)
				require.NotNil(t, cleanup.ReplyMarkup.InlineKeyboard)
				assert.Empty(t, cleanup.ReplyMarkup.InlineKeyboard)
				require.Len(t, tg.RequestsFor("sendMessage"), 1)
				assertTelegramExerciseResultReply(t, tg.RequestsFor("sendMessage")[0], telegramID, messageID+1, verdict)
				testkit.RequireStatus(t, telegramUpdate(t, update), http.StatusOK)
				require.Len(t, tg.RequestsFor("sendMessage"), 2)
				assert.JSONEq(t, string(tg.RequestsFor("sendMessage")[0].Body), string(tg.RequestsFor("sendMessage")[1].Body))
				assert.Empty(t, tg.RequestsFor("editMessageText"))
				assert.Empty(t, tg.RequestsFor("editMessageCaption"))
				testkit.RequireStatus(t, telegramUpdate(t, exerciseResultCallback(telegramID, messageID, "stale-skip", "idk:"+exercise.ID.String())), http.StatusOK)
				editAction := "editMessageText"
				if exerciseType == enums.ExerciseTypeAudioDirect || exerciseType == enums.ExerciseTypeAudioReversed {
					editAction = "editMessageCaption"
				}
				require.Len(t, tg.RequestsFor(editAction), 1)
				assertTelegramExerciseResultEdit(t, tg.RequestsFor(editAction)[0], telegramID, messageID, verdict)
				assert.Len(t, tg.RequestsFor("sendMessage"), 2)
				update["message"].(map[string]any)["message_id"] = messageID + 2
				testkit.RequireStatus(t, telegramUpdate(t, update), http.StatusOK)
				require.Len(t, tg.RequestsFor("sendMessage"), 3)
				assertTelegramExerciseResultReply(t, tg.RequestsFor("sendMessage")[2], telegramID, messageID+2, verdict)
				assert.Len(t, tg.RequestsFor(editAction), 1)
				assert.Equal(t, delta, *exerciseLink(t, exercise.ID, vocabulary.ID).ProgressDelta)
			})
		}
	}
}

func assertTelegramExerciseResultReply(t *testing.T, request testkit.TelegramRequest, chatID, replyToMessageID int64, verdict string) {
	t.Helper()
	var reply struct {
		ChatID           int64  `json:"chat_id"`
	...
	require.NoError(t, json.Unmarshal(request.Body, &reply))
	assert.Equal(t, chatID, reply.ChatID)
	assert.Equal(t, replyToMessageID, reply.ReplyToMessageID)
	assert.Equal(t, "Markdown", reply.ParseMode)
	assertTelegramExerciseResultText(t, reply.Text, verdict)
}

func assertTelegramExerciseResultEdit(t *testing.T, request testkit.TelegramRequest, chatID, messageID int64, verdict string) {
	t.Helper()
	var edit struct {
		ChatID      int64  `json:"chat_id"`
		MessageID   int64  `json:"message_id"`
	...
	require.NoError(t, json.Unmarshal(request.Body, &edit))
	assert.Equal(t, chatID, edit.ChatID)
	assert.Equal(t, messageID, edit.MessageID)
	assert.Equal(t, "Markdown", edit.ParseMode)
	require.NotNil(t, edit.ReplyMarkup.InlineKeyboard)
	assert.Empty(t, edit.ReplyMarkup.InlineKeyboard)
	text := edit.Text
	if request.Action == "editMessageCaption" {
		text = edit.Caption
	}
	assertTelegramExerciseResultText(t, text, verdict)
}

func assertTelegramExerciseResultText(t *testing.T, text, verdict string) {
	t.Helper()
	texts := telegram.GetBotTexts(enums.LanguageRu)
	feedback := map[string]string{"correct": texts.ExerciseSuccess, "almost": texts.ExerciseAlmost, "wrong": texts.ExerciseInvalid, "skipped": texts.ExerciseIDK}
	assert.Contains(t, text, feedback[verdict])
	assert.Contains(t, text, "carta")
	assert.Contains(t, text, "letter")
	assert.Contains(t, text, enums.LanguageIt.Flag())
	assert.Contains(t, text, enums.LanguageEn.Flag())
	assert.NotContains(t, text, "＿")
}

func TestTelegramWebhookChoiceAnswersEditExerciseMessage(t *testing.T) {
	for _, exerciseType := range []enums.ExerciseType{enums.ExerciseTypeChoiceDirect, enums.ExerciseTypeChoiceReversed} {
		for _, verdict := range []string{"correct", "wrong"} {
			t.Run(string(exerciseType)+"/"+verdict, func(t *testing.T) {
				testkit.Truncate(t)
				tg := testkit.MockTelegramAPI(t)
				const telegramID int64 = 555020
				const messageID int64 = 120
				user := testkit.CreateUser(t, testkit.WithTelegramID(telegramID))
				vocabularyIDs := make([]uuid.UUID, 0, 4)
				for _, pair := range [][2]string{{"carta", "letter"}, {"cane", "dog"}, {"gatto", "cat"}, {"casa", "house"}} {
					vocabulary := exerciseSeedVocabulary(t, user.ID, pair[0], pair[1], enums.LanguageIt, enums.LanguageEn)
					vocabularyIDs = append(vocabularyIDs, vocabulary.ID)
				}
				exercise := exerciseSeedChoiceExercise(t, user.ID, exerciseType, enums.ExerciseStatusInProgress, vocabularyIDs)
				require.NoError(t, db.DB.Model(&models.Exercise{}).Where("id = ?", exercise.ID).Update("telegram_message_id", messageID).Error)
				selected := vocabularyIDs[0]
				status := enums.ExerciseStatusCompleted
				if verdict == "wrong" {
					selected = vocabularyIDs[1]
					status = enums.ExerciseStatusFailed
				}
				update := exerciseResultCallback(telegramID, messageID, "choice-result", "answer:"+telegramCompactUUID(exercise.ID)+":"+telegramCompactUUID(selected))
				for range 2 {
					testkit.RequireStatus(t, telegramUpdate(t, update), http.StatusOK)
				}
				assert.Equal(t, status, exerciseReload(t, exercise.ID).Status)
				link := exerciseLink(t, exercise.ID, vocabularyIDs[0])
				require.NotNil(t, link.Result)
				assert.Equal(t, verdict, *link.Result)
				assert.Empty(t, tg.RequestsFor("sendMessage"))
				assert.Empty(t, tg.RequestsFor("editMessageReplyMarkup"))
				...
				for _, request := range tg.RequestsFor("answerCallbackQuery") {
					...
					require.NoError(t, json.Unmarshal(request.Body, &acknowledgement))
					assert.Equal(t, "choice-result", acknowledgement["callback_query_id"])
				}
			})
		}
	}
...
func exerciseResultCallback(telegramID, messageID int64, callbackID, action string) map[string]any {
	return map[string]any{
		"update_id": 200,
		"callback_query": map[string]any{
			"id":   callbackID,
	...
	for _, exerciseType := range []enums.ExerciseType{enums.ExerciseTypeCharactersDirect, enums.ExerciseTypeCharactersReversed} {
		for _, verdict := range []string{"correct", "almost", "wrong", "skipped"} {
			t.Run(string(exerciseType)+"/"+verdict, func(t *testing.T) {
				...
				tg := testkit.MockTelegramAPI(t)
				const telegramID int64 = 555021
				const messageID int64 = 121
				user := testkit.CreateUser(t, testkit.WithTelegramID(telegramID))
				vocabulary := exerciseSeedVocabulary(t, user.ID, "carta", "letter", enums.LanguageIt, enums.LanguageEn)
				exercise := exerciseSeedExercise(t, user.ID, exerciseType, enums.ExerciseStatusPending, vocabulary.ID)
				order := []int{0, 1, 2, 3, 4, 5, -1, -1, -1}
				if exerciseType == enums.ExerciseTypeCharactersReversed {
					order = []int{0, 1, 2, 3, 4, -1, -1, -1, -1}
				}
				require.NoError(t, services.StartCharacterExercise(exercise.ID, messageID, order))
				...
				case "wrong":
					...
					indices[1], indices[4] = indices[4], indices[1]
				case "skipped":
					indices = indices[:1]
				}
				...
				edits := tg.RequestsFor("editMessageText")
				require.NotEmpty(t, edits)
				assertTelegramExerciseResultEdit(t, edits[len(edits)-1], telegramID, messageID, verdict)
				link := exerciseLink(t, exercise.ID, vocabulary.ID)
				require.NotNil(t, link.Result)
...
func TestTelegramWebhookFailedAnswerDeliveryCanBeRetried(t *testing.T) {
	for _, exerciseType := range []enums.ExerciseType{
		...
		for _, verdict := range []string{"correct", "wrong", "skipped"} {
			...
			t.Run(string(exerciseType)+"/"+verdict, func(t *testing.T) {
				...
				require.NoError(t, db.DB.Model(&models.Exercise{}).Where("id = ?", exercise.ID).Update("telegram_message_id", messageID).Error)
				answer := "letter"
				if exerciseType == enums.ExerciseTypeCharactersReversed || exerciseType == enums.ExerciseTypeDescriptionDirect || exerciseType == enums.ExerciseTypeBasicReversed || exerciseType == enums.ExerciseTypeAudioReversed {
					answer = "carta"
				}
				if verdict == "wrong" {
					answer = "unrelated"
				...
				assert.Equal(t, expectedResult, *link.Result)
				require.Len(t, tg.RequestsFor(action), 1)
				firstDelivery := tg.RequestsFor(action)[0]
				if action == "sendMessage" {
					assertTelegramExerciseResultReply(t, firstDelivery, telegramID, messageID+1, verdict)
				} else {
					assertTelegramExerciseResultEdit(t, firstDelivery, telegramID, messageID, verdict)
				}
				testkit.RequireStatus(t, telegramUpdate(t, update), http.StatusOK)
				require.Len(t, tg.RequestsFor(action), 2)
				assert.JSONEq(t, string(firstDelivery.Body), string(tg.RequestsFor(action)[1].Body))
				for _, otherAction := range []string{"sendMessage", "editMessageText", "editMessageCaption"} {
					if otherAction != action {
						assert.Empty(t, tg.RequestsFor(otherAction))
					}
				}
				if action != "sendMessage" {
					assert.Empty(t, tg.RequestsFor("editMessageReplyMarkup"))
