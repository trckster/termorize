...
func TestDictionaryImportPromotesExistingWordsAndPreservesVocabulary(t *testing.T) {
	...
	assert.Equal(t, int64(6), job.Processed)
	assert.Equal(t, int64(2), job.Inserted)
	assert.Equal(t, int64(1), job.Classified)
	assert.Equal(t, int64(2), job.Skipped)
	assert.Equal(t, int64(1), job.Failed)
	assert.Equal(t, []string{"Line 6: invalid entry JSON or field types"}, job.RecordErrors)
	...
	assert.Equal(t, int64(5), repeated.Skipped)
