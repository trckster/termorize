...
func UpdateUserSettings(userID uint, settings models.UserSettings) (*models.User, error) {
	...
	err := db.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := cancelInProgressDescriptionExercises(tx, user.ID, newlyIgnoredDescriptionLanguages); err != nil {
			return err
		}
		...
		if wasDailyQuestionsEnabled && !settings.Telegram.DailyQuestionsEnabled {
			...
			if err := replacePendingDescriptionExercises(tx, user.ID, newlyIgnoredDescriptionLanguages); err != nil {
				return err
			}
		}
